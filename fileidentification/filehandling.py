...
from fileidentification.wrappers.wrappers import (
    Ffmpeg,
    Converter as Con,
    ImageMagick,
    Rsync,
...
from fileidentification.models import (
    SfInfo,
    BasicAnalytics,
    LogTables,
    LogMsg,
...
class FileHandler:
    ...
    def _integrity_test(self, sfinfo: SfInfo) -> None:
        ...
        if not puid:
            ...
            sfinfo.processing_logs.append(
                LogMsg(name="filehandler", msg="file removed")
            )
            msg = LogMsg(
                name="filehandler",
                msg=f"{FileProcessingMsg.PUIDFAIL} for {sfinfo.filename}",
            )
        ...
        if sfinfo.errors == FileDiagnosticsMsg.EMPTYSOURCE:
            self._remove(sfinfo)
            sfinfo.processing_logs.append(
                LogMsg(name="filehandler", msg=f"{FileDiagnosticsMsg.ERROR}")
            )
    ...
    def _apply_policy(self, sfinfo: SfInfo) -> None:
        ...
        if puid not in self.policies:
            ...
            if self.mode.STRICT:
                sfinfo.processing_logs.append(
                    LogMsg(name="filehandler", msg=f"{PolicyMsg.NOTINPOLICIES}")
                )
                self._remove(sfinfo)
                return
            # just flag it as skipped
            sfinfo.processing_logs.append(
                LogMsg(name="filehandler", msg=f"{PolicyMsg.SKIPPED}")
            )
            return
    ...
    def _is_file_corrupt(self, sfinfo: SfInfo) -> bool:
        ...
        match pbin:
            case Bin.FFMPEG:
                error, warning, specs = Ffmpeg.is_corrupt(
                    sfinfo, verbose=self.mode.VERBOSE
                )
                if specs and not sfinfo.media_info:
                    sfinfo.media_info.append(
                        LogMsg(name=Bin.FFMPEG, msg=json.dumps(specs))
                    )
                if warning:
                    ...
                    if any([msg in warning for msg in ErrMsgReencode]):
                        sfinfo.processing_logs.append(
                            LogMsg(name="filehandler", msg="re-encoding the file")
                        )
                        sfinfo.status.pending = True
            case Bin.MAGICK:
                error, warning, specs = ImageMagick.is_corrupt(  # type: ignore
                    sfinfo, verbose=self.mode.VERBOSE
                )
                if specs and not sfinfo.media_info:
    ...
    def _has_valid_streams(self, sfinfo: SfInfo) -> bool:
        ...
        if not streams:
            secho(
                f"\t{sfinfo.filename} throwing errors. consider to run script with flag -i [--integrity-tests]",
                fg=colors.RED,
                bold=True,
            )
            return True
    ...
    def _assert_policies(self) -> None:
        ...
        for el in self.policies:
            if self.policies[el]["bin"] not in Bin:
                print(
                    f"unknown bin {self.policies[el]['bin']} found in policy {el} ... exit"
                )
                raise typer.Exit(1)
            if not self.policies[el]["accepted"]:
                for k in [
                    ...
                    if k not in self.policies[el].keys():
                        print(
                            f"your policies missing field {k} in policy {el} ... exit"
                        )
                        raise typer.Exit(1)
                if ";" in self.policies[el]["processing_args"]:
                    print(
                        f"; not allowed in processing_args. found in policy {el} ... exit"
                    )
                    raise typer.Exit(1)

    def _gen_policies(
        self, outpath: Path, blank: bool = False, extend: bool = False
    ) -> None:
        """
        ...
        self.policies = generate_policies(
            outpath=outpath,
            ba=self.ba,
            fmt2ext=self.fmt2ext,
            strict=self.mode.STRICT,
            remove_original=self.mode.REMOVEORIGINAL,
            blank=blank,
            loaded_pol=loaded_pol,
        )
    ...
    def _test_policies(self, puid: str | None = None) -> None:
        ...
        if not puids:
            ...
            for puid in puids:
                ...
                if test:
                    est_time = (
                        self.ba.total_size[puid] / test.derived_from.filesize * duration  # type:ignore
                    )
    ...
    def _load_sfinfos(self, root_folder: Path) -> None:
        ...
        logpath_root = (
            f"{root_folder.parent}.{root_folder.stem}"
        ...
        if Path(f"{logpath_root}{JsonOutput.LOG}").is_file():
            for metadata in json.loads(
                Path(f"{logpath_root}{JsonOutput.LOG}").read_text()
            )["files"]:
        ...
        if not self.stack:
            with Progress(
                ...
                prog.add_task(
                    description="analysing files with siegfried...", total=None
                )
                for f in root_folder.rglob("*"):
                    if f.is_file():
                        self.stack.append(
                            SfInfo(
                                **pygfried.identify(f"{f}", detailed=True)["files"][0]  # type:ignore
                            )
                        )
                if root_folder.is_file():
                    self.stack.append(
                        SfInfo(
                            **pygfried.identify(f"{root_folder}", detailed=True)[  # type:ignore
                                "files"
                            ][0]
                        )
                    )
    ...
    def remove_tmp(self, root_folder: Path, to_csv: bool = False) -> None:
        ...
        with Progress(
            ...
            prog.add_task(
                description=f"moving files from {self.wdir.stem} to {root_folder.stem}...",
                total=None,
            )
    ...
    def _move_tmp(self) -> bool:
        ...
        for sfinfo in self.stack:
            ...
            if sfinfo.dest:
                ...
                if (
                    self.policies[sfinfo.derived_from.processed_as]["remove_original"]  # type: ignore
                    or self.mode.REMOVEORIGINAL
                ):
                    derived_from = [
                        sfi
                        for sfi in self.stack
                        if sfinfo.derived_from.filename == sfi.filename  # type: ignore
                    ][0]
                ...
                if abs_dest.is_file():
                    abs_dest = Path(
                        abs_dest.parent,
                        f"{sfinfo.filename.stem}_{sfinfo.md5[:6]}{sfinfo.filename.suffix}",
                    )
                ...
                if not sfinfo.filename.is_file():
                    sfinfo.filename = (
                        sfinfo.root_folder.parent / sfinfo.filename.relative_to("/data")
                    )
                # move the file
                ...
                if not err:
                    ...
                    self.log_tables.errors.append(
                        (LogMsg(name="rsync", msg=msg), sfinfo)
                    )
    ...
    def write_logs(self, root_folder: Path | str, to_csv: bool = False) -> None:
        ...
        with open(Path(f"{root_folder}{JsonOutput.LOG}"), "w") as f:
            f.write(
                logoutput.model_dump_json(indent=4, by_alias=True, exclude_none=True)
            )
    ...
    def run(
        ...
        # set the mode
        mode = Mode(
            REMOVEORIGINAL=remove_original,
            STRICT=mode_strict,
        ...
        root_folder = (
            Path(f"{root_folder.parent}.{root_folder.stem}")
            if root_folder.is_file()
            else root_folder
        )
        # generate policies
    ...
    def _set_working_dir(self, root_folder: Path, tmp_dir: Path | None = None) -> Path:
        ...
        if str(self.wdir) == str(Path.home()):
            self.wdir = Path(
                self.wdir / f"fileidentification_{datetime.now().strftime('%Y%m%d')}"
            )
...
class FileConverter:
    ...
    def _add_media_info(sfinfo: SfInfo, _bin: str) -> None:
        match _bin:
            ...
            case Bin.MAGICK:
                sfinfo.media_info.append(
                    LogMsg(
                        name="imagemagick", msg=ImageMagick.media_info(sfinfo.filename)
                    )
                )
            case _:
                pass
    ...
    def verify(target: Path, sfinfo: SfInfo, expected: list[str]) -> SfInfo | None:
        ...
        if target.is_file():
            ...
            target_sfinfo = SfInfo(
                **pygfried.identify(f"{target}", detailed=True)["files"][0]  # type:ignore
            )
            ...
            if target_sfinfo.processed_as in expected:
                ...
                p_error = (
                    f" did expect {expected}, got {target_sfinfo.processed_as} instead"
                )
                sfinfo.processing_logs.append(
                    LogMsg(
                        name="filehandler",
                        msg=f"{FileProcessingMsg.NOTEXPECTEDFMT}" + p_error,
                    )
                )
                secho(
                    f"\tERROR: {p_error} when converting {sfinfo.filename} to {target}",
                    fg=colors.YELLOW,
                    bold=True,
                )
                target_sfinfo = None
            ...
            sfinfo.processing_logs.append(
                LogMsg(name="filehandler", msg=f"{FileProcessingMsg.CONVFAILED}")
            )
            secho(
                f"\tERROR failed to convert {sfinfo.filename} to {target}",
                fg=colors.RED,
                bold=True,
            )
    ...
    def convert(self, sfinfo: SfInfo) -> tuple[SfInfo | None, list[str]]:
        ...
        logtext = (
            ...
            .replace(f"{sfinfo.root_folder}/", "")
            .replace(f"{sfinfo.wdir}/", "")
        )
        if logtext != "":
