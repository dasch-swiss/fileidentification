...
def inspect_file(sfinfo: SfInfo, policies: Policies, log_tables: LogTables, verbose: bool) -> None:
    ...
    if sfinfo.matches[0]["warning"] == FDMsg.EXTMISMATCH:
        if len(FMT2EXT[puid]["file_extensions"]) == 1:
            ...
            msg = f"expecting one of the following ext: {[el for el in FMT2EXT[puid]['file_extensions']]}"  # type: ignore
...
def _content_errors(sfinfo: SfInfo, policies: Policies, log_tables: LogTables, verbose: bool) -> bool:
    """
    checks if the file throws any error while opening or playing. error logging is added to the SfInfo class
    if the file fails completely, it's moved to _REMOVED. Only return True if there are major errors
